...
\begin{document}
	...
	\fanf	{Teams}
			{An einem Spiel sind 2 Teams beteiligt, die gegeneinander spielen. Diese beiden Teams haben einen Namen, ein Team-Motto, ein Logo und eine Hauptfarbe für das Trikot, aber auch eine Ersatzfarbe, falls das andere Team eine ähnliche Farbe hat, gegen das gespielt wird. Jedes Team besteht zudem aus 7 Spieler, wovon einer ein Sucher ist, einer ein Hüter, 2 sind sogenannte Treiber und die restlichen 3 sind Jäger.}
	...
	\fanf	{Partie-Konfiguration}
			{JSON Datei mit Informationen über eine Spiel.}
			{<Begründung>}
			{Spiellogik}
			{<Prio>}
	...
	\fanf	{Spielende}
			{Die Partie ist vorüber und es gibt einen Gewinner}
			{Bei bestimmt Ereignissen oder Gegebenheiten ist das Spiel zu ende und ein Sieger muss bestimmt werden.}
			{Spiellogik, Runde}
			{+}
			{Spiel, Spieler}
			
	\fanf	{Überlängenbehnadlung}
			{Reaktionen, welche getroffen werden, falls eine Partie zu lange läuft.}
			{Zieht sich ein Spiel über zu viele Runden hin, ohne dass ein Sieger ermittelt werden kann, so müssen laut Regelwerk Maßnahmen eingeleitet werden, damit die Partie zum Ende kommt.}
			{Spielende, Spiellogik, Runde}
			{-+}
			{Spiel, Spieler}
			
	\fanf	{Spiellogik}
			{Einheit, in der die Züge der Teams verarbeitet werden und auf Basis der Spielregeln im Lastenheft Entscheidungen getroffen werden.}
